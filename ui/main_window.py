...
class MainWindow(QMainWindow):
    ...
    def filter_templates(self,mode="전체 스타일"):
        ...
        for path,data in getattr(self,"template_entries",[]):
            ...
            if mode=="Signature Classic · 그와 그녀" and category!="signature_dual": continue
    ...
    def build_current_job_snapshot(self):
        ...
        if self.canvas_mode.currentIndex()==0 and self.template.get("category") in {"chill_rap_signature","signature_experimental_v2"}:width,height=int(profile.get("width",960)),int(profile.get("height",160))
