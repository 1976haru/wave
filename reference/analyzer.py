...
def _frame_style(frame,roi=None):
    ...
    b,g,r=color; return {"color":f"#{int(r):02X}{int(g):02X}{int(b):02X}","gradient_start":f"#{int(r):02X}{int(g):02X}{int(b):02X}","bands":int(np.clip(len(runs),8,256)),"bar_width":float(np.clip(bw/max(1,bw+gap),.05,1)),"gap":float(np.clip(gap/max(1,bw+gap),0,.95)),"position":"top" if center<.35 else "center" if center<.65 else "bottom","height":float(np.clip(np.ptp(ys)/h if ys.size else .2,.05,.9)),"glow":bool(cv2.Laplacian(value,cv2.CV_32F).var()<180),"glow_strength":float(np.clip(1-cv2.Laplacian(value,cv2.CV_32F).var()/500,0,1)),"mirror":bool(symmetry>.82)}
