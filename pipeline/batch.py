...
class BatchRunner:
    ...
    def run(self,files,output_dir,template,options=None,progress=None,render_fn=render_audio,skip_completed=False,state_file=None):
        ...
        for index,source in enumerate(files):
            ...
            try:
                details=render_fn(source,target,template,options,cancel=lambda:self.cancelled);entry.update(status="completed",completed_at=time.time(),error=None);results.append({"file":str(source),"status":"success",**(details or {})})
