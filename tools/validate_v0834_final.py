"""Generate the v0.8.3.4 final Tokyo Chill review package."""
...
OUT=ROOT/"validation_results/v0834_final_signature"
MODES=[
 ("Twin","Tokyo Twin Flow",ROOT/"templates/00_v0834_tokyo_chill_dual.json",ROOT/"templates/00_v0831_dual_twin_bloom.json",ROOT/"templates/00_v0832_twin_bloom_v2.json"),
 ("Midnight","Tokyo Midnight Flow",ROOT/"templates/00_v0834_tokyo_chill_his.json",ROOT/"templates/00_v0831_his_midnight_mirror.json",ROOT/"templates/00_v0832_midnight_grid_v2.json"),
 ("Pearl","Tokyo Pearl Flow",ROOT/"templates/00_v0834_tokyo_chill_her.json",ROOT/"templates/00_v0831_her_pearl_bloom.json",ROOT/"templates/00_v0832_pearl_bloom_v2.json")]
BACKGROUNDS=[
...
def main():
    ...
    report={"version":"0.8.3.4 BETA","engineering":"TECH PASS","visual_approval":"USER APPROVED DIRECTION / FINAL POLISH REVIEW","external_beta":"PACKAGE CANDIDATE","signatures":{},"checks":{}}
    ...
    for short,name,path,v1path,v2path in MODES:
        ...
        def frames():
            for i in range(720):
                ...
                yield frame
        video=OUT/"videos"/f"Tokyo_{short}_Flow_FINAL.mp4"; encode(video,frames());cpu_fps=720/(perf_counter()-started); strong=captures[600]
        ...
        history.append((f"{name} · v0.8.3.4 FINAL",strong)); gpu_fps=None
        try:
    ...
    for short,*_ in MODES:
        cap=cv2.VideoCapture(str(OUT/"videos"/f"Tokyo_{short}_Flow_FINAL.mp4"));cap.set(cv2.CAP_PROP_POS_FRAMES,600);ok,frame=cap.read();cap.release();mask=frame.max(axis=2)>18;ys,xs=np.where(mask);outside=np.ones(mask.shape,bool)
    ...
    # Distribution-friendly names requested for final review.
    shutil.copy2(OUT/"contact_sheets/V0833_MAIN_3.png",OUT/"contact_sheets/FINAL_SIGNATURE_3.png")
    shutil.copy2(OUT/"contact_sheets/V0833_BACKGROUND_12.png",OUT/"contact_sheets/FINAL_SIGNATURE_3_BACKGROUNDS.png")
    subtitle_items=[(label,img) for label,img in mockups if "TokyoNight" in label]
    contact(subtitle_items,OUT/"contact_sheets/FINAL_SIGNATURE_3_WITH_SUBTITLES.png",cols=3,cell_h=582)
    text=json.dumps(report,ensure_ascii=False,indent=2);(OUT/"reports/final_signature_report.json").write_text(text,encoding="utf-8");(OUT/"reports/final_signature_report.txt").write_text(text,encoding="utf-8");print(text)
