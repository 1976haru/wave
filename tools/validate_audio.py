...
def probe(path):
    ...
    process=subprocess.run([shutil.which("ffprobe"),"-v","error","-select_streams","a:0","-show_entries","stream=sample_rate,channels,duration","-of","json",str(path)],capture_output=True,text=True)
    try:return json.loads(process.stdout).get("streams",[{}])[0]
    except Exception:return {}
def preview_benchmark(features,template,frames=48):
    renderer=CPURenderer();engine=AnimationEngine(features,template);started=time.perf_counter()
...
def validate(root,output="validation_results/validation_report.json",limit=15):
    root=Path(root);files=sorted(p for p in root.rglob("*") if p.suffix.lower() in EXTENSIONS);files=files if limit==0 else files[:limit];results=[];total_started=time.perf_counter();settings=AnalysisSettings(fps=24,bands=48);cache_dir=Path("validation_results/cache");template=load_template("templates/01_clean_bars.json")
    for source in files:
        ...
        try:
            ...
            cold=time.perf_counter();features,_=analyze_file(source,settings,cache_dir);cold=time.perf_counter()-cold;warm=time.perf_counter();_,hit=analyze_file(source,settings,cache_dir);warm=time.perf_counter()-warm;fps=preview_benchmark(features,template);peak=tracemalloc.get_traced_memory()[1]/(1024*1024);estimated_fps=max(.1,fps*(480*270)/(1920*1080));item.update(status="success",decode_success=True,duration_seconds=duration,sample_rate=int(metadata.get("sample_rate",sr)),channels=int(metadata.get("channels",1)),analysis_cache_miss_seconds=cold,analysis_cache_hit_seconds=warm,cache_hit=hit,cache_speedup=cold/max(warm,1e-9),peak_memory_mb=peak,preview_fps=fps,estimated_final_fps=estimated_fps,estimated_final_render_seconds=duration/estimated_fps*30)
