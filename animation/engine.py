...
class AnimationEngine:
    ...
    def __init__(self, features, template):
        ...
        if str(template.get("renderer","")).lower()=="stereo_signature" and (template.get("adaptive_normalization") or template.get("signature_tier")=="FINAL"):
            ...
            self.signature_calibration=(low,np.maximum(high-low,float(template.get("calibration_min_span",.035))))
    def reset(self): self.prev = None
    def sample(self, seconds, absolute_seconds=None):
        ...
        if self.signature_calibration is not None:
            low,span=self.signature_calibration;calibrated=np.clip((values-low)/span,0,1);blend=np.clip(float(self.t.get("adaptive_blend",.58)),0,1);values=(1-blend)*values+blend*calibrated
        ...
        if str(self.t.get("renderer", "")).lower() in {"dot_matrix", "twin_dot_matrix", "dot_line_hybrid", "echo_dots", "stereo_signature"}:
            ...
            default_body_floor=.12 if self.t.get("signature_tier")=="FINAL" else floor
            values=np.maximum(values,float(self.t.get("signature_body_floor",default_body_floor)))
